...
import (
	...
	"github.com/wyverny/sqlboiler/v4/drivers/mocks"
)
...
func testNew(t *testing.T, aliases Aliases) {
	...
	cmd = exec.Command("go", "mod", "edit", fmt.Sprintf("-replace=github.com/wyverny/sqlboiler/v4=%s", filepath.Dir(string(goModFilePath))))
	cmd.Dir = state.Config.OutFolder
	cmd.Stderr = buf
